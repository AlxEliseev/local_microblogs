...
class Base(DeclarativeBase):
    def __repr__(self):
        columns = [column.key for column in inspect(self.__class__).columns]
        attrs = {attr: getattr(self, attr) for attr in columns}
        attr_lst = [f"{a_name}={a_value!r}" for a_name, a_value in attrs.items()]
...
class User(Base):
    ...
    followers: Mapped[List["User"]] = association_proxy(
        "follower_links", "follower", creator=lambda user: Follow(follower=user)
    )
    ...
    following: Mapped[List["User"]] = association_proxy(
        "following_links", "followee", creator=lambda user: Follow(followee=user)
    )
    ...
    liked_tweets: Mapped[List["Tweet"]] = association_proxy(
        "likes", "tweet", creator=lambda tweet: Like(tweet=tweet)
    )
